...
function [bestFitnessArray] = TBO_multi_branch(x,y,z, ParticlesNum, step, GlobalIterationNum, disp_figures_and_results, sub_algorithm)

x_min = min(x);
x_max = max(x);
y_min = min(y);
y_max = max(y);
ParticlesNumEntered = ParticlesNum;
...
for globalIteration = 1:GlobalIterationNum
    ...
    iterationLocalSearch_simpleLocalSearch = 100;

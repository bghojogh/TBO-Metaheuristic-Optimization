...
%% taking sub_algorithm from user:
sub_algorithm = input('Please enter sub-algorithm (1, 2 or 3): ');
...
sum_error = zeros(3,1);
fitness_plot_TBO_Binary = zeros(2, GlobalIterationNum, length_of_i_for * length_of_j_for * length_of_k_for);  %% rows: x and y of plot (x: iteration, y: fitness) , colomns: iterations that better answer is found: at most (worst) in all iterations we have better answer , third dimension: each test on algorithm
fitness_plot_TBO_multi_branch = zeros(2, GlobalIterationNum, length_of_i_for * length_of_j_for * length_of_k_for);
fitness_plot_TBO_adaptive = zeros(2, GlobalIterationNum, length_of_i_for * length_of_j_for * length_of_k_for);
...
for i = start_of_i_for:stop_of_i_for            %%% different numbers of population individuals
    ...
    for j = 1:5                                 %%% different locations of population individuals
        ...
        for k = 1:5                             %%% different executions of algorithms
            ...
            disp(' ');
            %% TBO_Binary:
            cd('./algorithms/Binary_TBO');
            [bestFitnessArray] = TBO_Binary(x,y,z, PopulationNum, step, GlobalIterationNum, disp_figures_and_results, sub_algorithm);
            cd('..');
            cd('..');
            ...
            fitness_plot_TBO_Binary(:,1:size(bestFitnessArray,2),counter2) = [bestFitnessArray(4,:) ; bestFitnessArray(1,:)];
            numberOfIterations_that_better_answer_found(1,counter2) = size(bestFitnessArray,2);
            
            %% TBO_multi_branch:
            %close all
            cd('./algorithms/Multi_branch_TBO');
            [bestFitnessArray] = TBO_multi_branch(x,y,z, PopulationNum, step, GlobalIterationNum, disp_figures_and_results, sub_algorithm);
            cd('..');
            cd('..');
            ...
            fitness_plot_TBO_multi_branch(:,1:size(bestFitnessArray,2),counter2) = [bestFitnessArray(4,:) ; bestFitnessArray(1,:)];
            numberOfIterations_that_better_answer_found(2,counter2) = size(bestFitnessArray,2);

            %% TBO_adaptive:
            %close all
            cd('./algorithms/Adaptive_TBO');
            [bestFitnessArray] = TBO_adaptive(x,y,z, PopulationNum, step, GlobalIterationNum, disp_figures_and_results, sub_algorithm);
            cd('..');
            cd('..');
            ...
            fitness_plot_TBO_adaptive(:,1:size(bestFitnessArray,2),counter2) = [bestFitnessArray(4,:) ; bestFitnessArray(1,:)];
...
%%% equaling length of fitness_plot_TBO_Binary for all testings (so we can take a mean of them), and also ending the plots to GlobalIterationNum:
...
for j=1:length_of_i_for * length_of_j_for * length_of_k_for
    for i=1:max(numberOfIterations_that_better_answer_found(1,:))+1  % max + 1 --> because we want to end all of them in GlobalIterationNum
        if fitness_plot_TBO_Binary(1,i,j) == 0
            fitness_plot_TBO_Binary(1,i:GlobalIterationNum,j) = GlobalIterationNum;
            fitness_plot_TBO_Binary(2,i:GlobalIterationNum,j) = fitness_plot_TBO_Binary(2,i-1,j);
            break;
        end
    end
end
...
for j=1:length_of_i_for * length_of_j_for * length_of_k_for
    for i=1:max(numberOfIterations_that_better_answer_found(2,:))+1  % max + 1 --> because we want to end all of them in GlobalIterationNum
        if fitness_plot_TBO_multi_branch(1,i,j) == 0
            fitness_plot_TBO_multi_branch(1,i:GlobalIterationNum,j) = GlobalIterationNum;
            fitness_plot_TBO_multi_branch(2,i:GlobalIterationNum,j) = fitness_plot_TBO_multi_branch(2,i-1,j);
            break;
        end
    end
end
...
for j=1:length_of_i_for * length_of_j_for * length_of_k_for
    for i=1:max(numberOfIterations_that_better_answer_found(3,:))+1  % max + 1 --> because we want to end all of them in GlobalIterationNum
        if fitness_plot_TBO_adaptive(1,i,j) == 0
            fitness_plot_TBO_adaptive(1,i:GlobalIterationNum,j) = GlobalIterationNum;
            fitness_plot_TBO_adaptive(2,i:GlobalIterationNum,j) = fitness_plot_TBO_adaptive(2,i-1,j);
            break;
        end
    end
end
...
hold on
for j=1:length_of_i_for * length_of_j_for * length_of_k_for 
    plot(fitness_plot_TBO_Binary(1,:,j), fitness_plot_TBO_Binary(2,:,j));
end
hold off
title('Binary TBO');
plot_data = get(get(gca,'Children'),'YData'); % cell array of all "y" data of plots
average_TBO_Binary = mean(cell2mat(plot_data));

%%% for TBO_multi_branch:
figure
hold on
for j=1:length_of_i_for * length_of_j_for * length_of_k_for 
    plot(fitness_plot_TBO_multi_branch(1,:,j), fitness_plot_TBO_multi_branch(2,:,j));
end
hold off
title('Multi-branch TBO');
plot_data = get(get(gca,'Children'),'YData'); % cell array of all "y" data of plots
average_TBO_multi_branch = mean(cell2mat(plot_data));

%%% for TBO_adaptive:
figure
hold on
for j=1:length_of_i_for * length_of_j_for * length_of_k_for 
    plot(fitness_plot_TBO_adaptive(1,:,j), fitness_plot_TBO_adaptive(2,:,j));
end
hold off
title('Adaptive TBO');
plot_data = get(get(gca,'Children'),'YData'); % cell array of all "y" data of plots
average_TBO_adaptive = mean(cell2mat(plot_data));

%%%%
figure
hold on
plot(average_TBO_Binary, 'r', 'LineWidth',3);
plot(average_TBO_multi_branch, '--b', 'LineWidth',3);
plot(average_TBO_adaptive, ':k', 'LineWidth',3);
l = legend('Binary TBO', 'Multi-branch TBO', 'Adaptive TBO');
set(l, 'FontSize', 15);
a = min([min(average_TBO_Binary), min(average_TBO_multi_branch), min(average_TBO_adaptive)]);
b = max([max(average_TBO_Binary), max(average_TBO_multi_branch), max(average_TBO_adaptive)]);
ylim([a-((b-a)*0.05) , b+((b-a)*0.05)]);
switch sub_algorithm
    case 1
        str = sprintf('sub-algorithm: LS, No. particles: %d, benchmark: %d', numberOfParticles, numberOfBenchmark);
    case 2
        str = sprintf('sub-algorithm: PSO, No. particles: %d, benchmark: %d', numberOfParticles, numberOfBenchmark);
    case 3
        str = sprintf('sub-algorithm: GA, No. particles: %d, benchmark: %d', numberOfParticles, numberOfBenchmark);
end
t = title(str);
